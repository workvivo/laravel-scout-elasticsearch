...
return [
    ...
    'wait_no_batch' => 'Dispatched [:searchable] to connection [:connection], queue [:queue], but no batch appeared within :elapsed s (SCOUT_IMPORT_WAIT_TIMEOUT=:timeout). It is still queued.',
    'wait_no_batch_hint' => 'Confirm a queue:work is running for that connection/queue, or raise SCOUT_IMPORT_WAIT_TIMEOUT if planning chunks is slow on this table (try --fast-plan or a larger --chunk).',
